...
class FLOW2(Searcher):
    ...
    def lexico_compare(self, result) -> bool:
        ...
        # print("-------------------------------------")
        # print("result", result)
        # print("lexico", self.lexico_best)
        # print("-------------------------------------")
        if self.lexico_best is None:
            self.lexico_best = {
                k: result[k] for k in self.lexico_info["metric_priority"]
            }
            return True
        for k_metric in self.lexico_info["metric_priority"]:
            ...
            B_lower = (
                self.lexico_best[k_metric]
                if self.lexico_best[k_metric] > k_c
                else float("-inf")
            )
            B_upper = (
                self.lexico_best[k_metric] + k_T
                if self.lexico_best[k_metric] > k_c
                else k_c
            )
            # print("--------")
            ...
            if result[k_metric] >= B_lower and result[k_metric] <= B_upper:
                continue
            elif result[k_metric] > B_upper:
                return False
            elif result[k_metric] < B_lower:
                up_index = self.lexico_info["metric_priority"].index(k_metric)
                for up_metr in (self.lexico_info["metric_priority"])[up_index:]:
    ...
    def update_lexico(self,):
        obj1 = self.lexico_info["metric_priority"][0]
        feasible_index = [*range(len(self.histories[obj1]))]
        # print("-----------------------------------------------------------------------------")
        for k in self.lexico_info["metric_priority"]:
            # print(k)
            # print(self.histories[k])
            # print(feasible_index)
            tem_values = np.array(self.histories[k])
            min_value = np.min(tem_values.take(feasible_index))
            self.lexico_best[k] = min_value
            feasible_index_0 = np.where(tem_values <= min_value + self.lexico_info["tolerance"][k])[0].tolist()
            feasible_index = [val for val in feasible_index if val in feasible_index_0]
        # print("-----------------------------------------------------------------------------")

    def lexico_compare_thoroughly(self, result) -> bool:
        """Compare with incumbent over lexicographic preference"""
        print("-------------------------------------")
        print("result", result)
        print("f_best", self.lexico_best)
        ...
        if self.histories is None:
            ...
            for k_metric in self.lexico_info["metric_priority"]:
                k_T = self.lexico_info["tolerance"][k_metric]
                k_c = self.lexico_info["target"][k_metric]
                if (result[k_metric] < max([self.lexico_best[k_metric] + k_T, k_c])) and (self.best_obj[k_metric] < max([self.lexico_best[k_metric] + k_T, k_c])):
                    continue
    ...
    def create(
        ...
        if self.lexico_info is not None:
            flow2.best_obj = {}
            # print(obj)
            for k, v in obj.items():
                flow2.best_obj[k] = v * self.metric_op
        else:
    ...
    def on_trial_complete(
        ...
        if not error and result:
            obj = (
                ...
                else {k: result[k] for k in self.lexico_info["metric_priority"]}
            )
            if obj:
                obj = (
                    {
                        k: obj[k] * self.metric_op
                        for k in self.lexico_info["metric_priority"]
                    }
                ...
                if (
                    ...
                    or (self.lexico_info is None and obj < self.best_obj)
                    or (self.lexico_info is not None and (self.lexico_compare(obj) if self.lexico_info["version"] == 1 else self.lexico_compare_thoroughly(obj)))
                ):
                    # print(self.best_obj)
                    self.best_obj = obj
    ...
    def converged(self) -> bool:
        ...
        # print("step", self.step_lower_bound)
        # print("---------------------------------")
        return self.step < self.step_lower_bound
        # if self.step < 0.24:
        #     for i in range(500):
        #         print("hello")
        # return self.step < 0.24
...
# 1662239733.937744 - 1662239642.273693
