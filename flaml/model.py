...
class BaseEstimator:
    ...
    * LGBMEstimator, RandomForestEstimator, LRL1Classifier, LRL2Classifier:
    ...
    def __init__(self, task="binary", live_estimator = None, **config):
        """Constructor.
        ...
        if live_estimator:
            self.model_add = live_estimator.get_booster()
        else:
            self.model_add = None
    ...
    def _fit(self, X_train, y_train, **kwargs):
        ...
        kwargs["xgb_model"] = self.model_add
        ...
        # print("booster",self._model.get_booster().num_boosted_rounds())
        kwargs["early_stopping_rounds"] = None
        ...
        if not self.HAS_CALLBACK:
            ...
            if (
                ...
                if (
                    ...
                    return time.time() - start_time, self._model
                trained = True
            # logger.debug(mem0)
...
class MyEstimator(XGBoostSklearnEstimator):
    def __init__(
        self,
        task="binary",
        **config,
    ...
    def _fit(self, X_train, y_train, **kwargs):
        ...
        if "groups" in kwargs:
            ...
            if self._task == "rank":
                ...
                #     del kwargs['groups_val'], kwargs['X_val'], kwargs['y_val']
        X_train = self._preprocess(X_train)
        if "feature_num" in self.params.keys():
            feature_num = self.params["feature_num"]
    ...
    def predict(self, X, **kwargs):
        if "feature_num" in kwargs.keys():
            del kwargs["feature_num"]
    ...
    def predict_proba(self, X, **kwargs):
        if "feature_num" in kwargs.keys():
            del kwargs["feature_num"]
        result = super().predict_proba(X, **kwargs)
        return result

    def score(self, X_val: DataFrame, y_val: Series, **kwargs):
        if "feature_num" in kwargs.keys():
            del kwargs["feature_num"]
        result = super().score(X_val, y_val, **kwargs)
        return result
...
class Myforest(RandomForestEstimator):
    def __init__(
        self,
        task="binary",
        **params,
