...
class AutoMLState:
    ...
    @staticmethod
    def _compute_with_config_base(config_w_resource, state, estimator, live_estimator = None):
        # print("------------------------")
        # print(config_w_resource)
        # print("------------------------")
        ...
        fairness_info = state.fairness_info
        (
            trained_estimator,
            val_loss,
            metric_for_logging,
        ...
        if "get_stable" not in config_w_resource.keys():
            st = None
        ...
        result = {
            ...
            "feature_num": config_w_resource["feature_num"] if "feature_num" in config_w_resource.keys() else None,
            "fairness": metric_for_logging["fairness_metric"] if state.fairness_info else None,
            "val_loss": val_loss,
            "stability" : 1 - st if st != None else None,
...
class AutoML(BaseEstimator):
    ...
    def trainable(self) -> Callable[[dict], Optional[float]]:
        ...
        def train(config: dict,state,live_estimator = None):
            ...
            if states[estimator].learner_class.size(config) <= mem_res:
                del config["learner"]
                if self._state.eval_only:
                    result, live_model = AutoMLState._compute_with_config_base(
                        config, state=state, estimator=estimator, live_estimator=live_estimator
                    ) if self.eval_only is False else AutoMLState._compute_with_config_base(config, state=state, estimator=estimator)
                else:
