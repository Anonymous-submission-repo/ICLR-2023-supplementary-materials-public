...
def get_estimator_class(task, estimator_name):
    ...
    if "xgboost" == estimator_name:
        ...
        # estimator_class = XGBoost_TS if task in TS_FORECAST else MyEstimator
...
def _eval_estimator(
    ...
    def fair_metric_func(fairness_metric, sensitive_attr, X_test, y_test_pred, y_test):
        from fairlearn.metrics import demographic_parity_difference, equalized_odds_difference
        sensitive_features_test = X_test[sensitive_attr]
        fairness_metrics_to_log = {}
        if 'EO' == fairness_metric:
            fairlearn_metric = equalized_odds_difference
        elif 'DSP' == fairness_metric:
            fairlearn_metric = demographic_parity_difference
        fairnes_score_on_test = fairlearn_metric(y_test, y_test_pred, sensitive_features=sensitive_features_test) 
        fairness_metrics_to_log["fairness_metric"] = fairnes_score_on_test
        return fairness_metrics_to_log
    if isinstance(eval_metric, str):
        pred_start = time.time()
        val_pred_y = get_y_pred(estimator, X_val, eval_metric, obj)
        if fairness_info != None:
            real_y = estimator.predict(X_val)
            fair_metric = fairness_info["fair_metric"]
            sensitive_attr = fairness_info["sensitive_attr"]
            fairness_metrics_to_log = fair_metric_func(fair_metric, sensitive_attr, X_val, real_y, y_val)
    ...
    if fairness_info:
        metric_for_logging.update(fairness_metrics_to_log)
...
def get_val_loss(
    ...
    def preprocess(X):
        if isinstance(X, DataFrame):
            ...
            X = X.to_numpy()
        return X
    st_X_train = preprocess(X_train)
    length, feature_max = st_X_train.shape[0],st_X_train.shape[1]
    st = None
    if "get_stable" in config.keys():
        feature_num = int(config["feature_num"])
        z = np.zeros((50,feature_max))
        for i in range(50):
            selector = SelectKBest(chi2, k=feature_num)
            newData=getBootstrapSample(st_X_train,y_train)
            selector.fit(newData['bootData'],newData['bootLabels'])
    ...
    tem_train_time, live_model = estimator.fit(X_train, y_train, budget, **fit_kwargs)
    ...
    return val_loss, metric_for_logging, train_time, pred_time, st, live_model
...
def evaluate_model_CV(
    ...
    whether_st = "get_stable" in config.keys()
    val_loss_folds = []
    log_metric_folds = []
    metric = None
    ...
    for train_index, val_index in kf:
        ...
        if groups is not None:
            ...
            groups_val = None
        val_loss_i, metric_i, train_time_i, pred_time_i, st_i, live_model_i = get_val_loss(
            config,
            estimator,
            X_train,
            y_train,
    ...
    if whether_st:
        ...
        st = None
    return val_loss, metric, train_time, st, pred_time
...
def compute_estimator(
    ...
    if "holdout" == eval_method:
        val_loss, metric_for_logging, train_time, pred_time, st, live_model = get_val_loss(
            config_dic,
            estimator,
            X_train,
            y_train,
        ...
        val_loss, metric_for_logging, train_time, st, pred_time = evaluate_model_CV(
            config_dic,
            estimator,
            X_train,
            y_train,
    ...
    return estimator, val_loss, metric_for_logging, train_time, pred_time, st, live_model
